...
for ($pid = 1; $pid <= $maxpid; ++$pid) {
	...
	$where = "WHERE p.pokemon_id = '".$pid."' AND (UNIX_TIMESTAMP(p.disappear_time) - (LENGTH(s.spawndef) - LENGTH( REPLACE ( spawndef, \"s\", \"\") )) * 900) > '".$last_update."'";
	$req = "SELECT count, (UNIX_TIMESTAMP(p.disappear_time) - (LENGTH(s.spawndef) - LENGTH( REPLACE ( spawndef, \"s\", \"\") )) * 900) as last_timestamp, (CONVERT_TZ(p.disappear_time, '+00:00', '".$time_offset."')) AS disappear_time_real, p.latitude, p.longitude
		FROM pokemon p
		JOIN trs_spawn s ON p.spawnpoint_id = s.spawnpoint
		JOIN (
