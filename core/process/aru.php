...
switch ($request) {
    ...
    case 'trainer':
        ...
        if (isset($_GET['team']) && 0 != $_GET['team']) {
            $team = mysqli_real_escape_string($mysqli, $_GET['team']);
            $where .= ' AND team = '.$team;
        }
        if (!empty($config->system->trainer_blacklist)) {
            $where .= " AND name NOT IN ('".implode("','", $config->system->trainer_blacklist)."')";
        }
        if (isset($_GET['page'])) {
    ...
    switch ($postRequest) {
        case 'pokemon_live':
            ...
            if (isset($_POST['pokemon_id'])) {
                ...
                if (null != $testIv->iv && isset($_POST['ivMin']) && ('' != $_POST['ivMin'])) {
                ...
                if (null != $testIv->iv && isset($_POST['ivMax']) && ('' != $_POST['ivMax'])) {
