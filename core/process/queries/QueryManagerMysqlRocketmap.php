...
final class QueryManagerMysqlRocketmap extends QueryManagerMysql
{
    ...
    public function getAllPokestops($only_lured = false)
    {
        $req = "SELECT latitude, longitude, lure_expiration, UTC_TIMESTAMP() AS now,
				...
				FROM pokestop";
        if ($only_lured) {
            $req .= ' WHERE lure_expiration > UTC_TIMESTAMP()';
        }
        $result = $this->mysqli->query($req);
        $pokestops = array();
        while ($data = $result->fetch_object()) {
    ...
    public function getGymShaver($gym_name, $team, $page, $ranking)
    {
        ...
        switch ($ranking) {
            case 1:
                ...
                break;
            case 2:
                $order = ' ORDER BY total_cp_end DESC, last_modified_end DESC';
                break;
            default:
                $order = ' ORDER BY last_modified_end DESC, name';
        }
        $limit = ' LIMIT '.($page * 5).',5';
        $req = "SELECT gym_id, name, team_id, total_cp_end, total_cp_start,
                pokemon_count_end, pokemon_count_start, pokemon_uids_end, pokemon_uids_start,
                CONVERT_TZ(last_modified_end, '+00:00', '".self::$time_offset."') AS last_modified_end,
                CONVERT_TZ(last_modified_start, '+00:00', '".self::$time_offset."') AS last_modified_start
                FROM gymshaving
                ".$where.$order.'
                LIMIT '.($page * 5).',5';
        $result = $this->mysqli->query($req);
        ...
        while ($data = $result->fetch_object()) {
            ...
            $data->total_cp_diff = $data->total_cp_end - $data->total_cp_start;
            $data->class = $data->total_cp_diff > 0 ? 'gain' : ($data->total_cp_diff < 0 ? 'loss' : '');
            ...
            $pkm_result = $this->mysqli->query($pkm_req);
    ...
    public function getGymShaverCount()
    {
        ...
        $trainer_result = $this->mysqli->query($trainer_req);
        ...
        $trainer_result = $this->mysqli->uery($trainer_req);
        while ($trainer_result && $trainer_data = $trainer_result->fetch_object()) {
            $entry = $trainer_data;
    ...
    private function getTrainerData($trainer_name, $team, $page, $ranking)
    {
        ...
        if (!empty(self::$config->system->trainer_blacklist)) {
            $where .= " AND name NOT IN ('".implode("','", self::$config->system->trainer_blacklist)."')";
        }
        if ('' != $trainer_name) {
    ...
    private function getTrainerActivePokemon($trainer_name)
    {
        $req = "SELECT DISTINCT gympokemon.pokemon_id, gympokemon.pokemon_uid, gympokemon.cp,
				...
				filtered_gymmember.name as gym_name,
				'1' AS active
				...
				GROUP BY gymmember.pokemon_uid, gymmember.deployment_time, gymmember.gym_id HAVING gymmember.gym_id <> '') AS filtered_gymmember
